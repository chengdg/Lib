...
class PermissionModeratorTests(CMSTestCase):
    ...
    def setUp(self):
        ...
        self.user_main = self._create_user("main", is_staff=True,
                                             add_default_permissions=True)
        self.user_subordinate = self._create_user("subordinate", is_staff=True,
                                            add_default_permissions=True)
        ...
        with self.login_user_context(self.user_super):
            ...
            self.main_page = create_page("main", "nav_playground.html", "en")
            ...
            self.subordinate_page = create_page("subordinate-home", "col_two.html", "en",
            ...
            publish_page(self.main_page, self.user_super, 'en')
    ...
    def test_main_cannot_add_page_to_root(self):
        with self.login_user_context(self.user_main):
            response = self.client.get(URL_CMS_PAGE_ADD)
            self.assertEqual(response.status_code, 403)

    def test_subordinate_cannot_add_page_to_root(self):
    ...
    def test_subordinate_can_add_page_under_subordinate_home(self):
        with self.login_user_context(self.user_subordinate):
            ...
            publish_page(page, self.user_subordinate, 'en')
            # user_subordinate is moderator for this page
            # approve / publish as user_subordinate
            # user main should be able to approve as well
    ...
    def test_default_plugins(self):
        with self.login_user_context(self.user_subordinate):
            ...
            response = self.client.get(self.subordinate_page.get_absolute_url(), {'edit': 1})
            self.assertEqual(response.status_code, 200)
            self.assertEqual(CMSPlugin.objects.count(), 1)

    def test_page_added_by_subordinate_can_be_published_by_user_main(self):
        # add page
        page = create_page("page", "nav_playground.html", "en",
                           parent=self.subordinate_page, created_by=self.user_subordinate)
        ...
        # should be True user_main should have publish permissions for children as well
        publish_page(self.subordinate_page, self.user_main, 'en')
        page = publish_page(page, self.user_main, 'en')
    ...
    def test_create_copy_publish(self):
        ...
        page = publish_page(copied_page, self.user_main, 'en')
        self.check_published_page_attributes(page)
    ...
    def test_create_publish_copy(self):
        ...
        self.check_published_page_attributes(page)
        copied_page = publish_page(copied_page, self.user_main, 'en')
    ...
    def test_subtree_needs_approval(self):
        ...
        subpage = publish_page(subpage, self.user_main, 'en')
    ...
    def test_moderator_flags(self):
        ...
        # check publish box
        page = publish_page(page, self.user_subordinate, 'en')
        ...
        self.subordinate_page = self.subordinate_page.reload()
        subordinate_page = publish_page(self.subordinate_page, self.user_main, 'en')

        self.assertFalse(page.publisher_public)
        self.assertTrue(subordinate_page.publisher_public)
    ...
    def test_remove_plugin_page_under_moderation(self):
        ...
        publish_page(subordinate_page, self.user_main, 'en')
        page = self.reload(page)
        page = publish_page(page, self.user_main, 'en')
...
class PatricksMoveTest(CMSTestCase):
    ...
    def setUp(self):
        ...
        with self.login_user_context(self.user_super):
            ...
            # create main user
            self.user_main = self._create_user("main", True)
            self.user_main.user_permissions.add(Permission.objects.get(codename='publish_page'))
            ...
            self.subordinate_page = create_page("subordinate-home", "nav_playground.html", "en",
                                          parent=self.main_page, created_by=self.user_super)
            subordinate = self._create_user("subordinate", True)
        ...
        with self.login_user_context(self.user_subordinate):
            ...
            self.pa = create_page("pa", "nav_playground.html", "en", parent=self.subordinate_page)
            ...
            publish_page(self.pa, self.user_main, 'en')
            publish_page(self.pb, self.user_main, 'en')
            publish_page(self.pc, self.user_main, 'en')
            publish_page(self.pd, self.user_main, 'en')
            publish_page(self.pe, self.user_main, 'en')
            publish_page(self.pf, self.user_main, 'en')
            publish_page(self.pg, self.user_main, 'en')
            publish_page(self.ph, self.user_main, 'en')
            self.reload_pages()
    ...
    def test_patricks_move(self):
        ...
        # perform moves under subordinate...
...
class GlobalPermissionTests(CMSTestCase):
    ...
    def test_emulate_admin_index(self):
        ...
        homepage = create_page(title="main", template="nav_playground.html",
