...
class MongoClient(common.BaseObject):
    ...
    def __try_node(self, node):
        ...
        type. Returns ((host, port), ismain, isdbgrid, res_time).
        ...
        return node, response['ismain'], isdbgrid, res_time
    ...
    def __find_node(self, seeds=None):
        ...
        True if this is a primary or main, else False. Sets __is_mongos
        ...
        for candidate in candidates:
            try:
                node, ismain, isdbgrid, res_time = self.__try_node(candidate)
                self.__is_primary = ismain
        ...
        for candidate in self.__nodes - candidates:
            try:
                node, ismain, isdbgrid, _ = self.__try_node(candidate)
                self.__is_primary = ismain
                self.__is_mongos = isdbgrid
                return node
            except Exception, why:
